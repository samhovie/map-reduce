...
def test_worker_01_register(mocker):
    ...
    mockclientsocket.recv.side_effect = [
        # Fake the first message sent by the master.  When the student worker
        # calls recv(), it will receive a register acknowledgement.
        json.dumps({
        ...
        # None value terminates recv loop
        None,

        # Fake the second message sent by the master.  When the student worker
